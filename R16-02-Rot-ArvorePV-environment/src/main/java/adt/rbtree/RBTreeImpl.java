...
public class RBTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements RBTree<T> {
	...
	private boolean verifyChildrenOfRedNodes(RBNode<T> node) {
		...
		if (!node.isEmpty()) {
			if (node.getColour() == Colour.RED && (castNode(node.getLeft()).getColour() == Colour.RED
					|| castNode(node.getRight()).getColour() == Colour.RED)) {
				isLeftValid = false; 
				isRightValid = false;
			} else {
				isLeftValid = verifyChildrenOfRedNodes(castNode(node.getLeft()));
				isRightValid = verifyChildrenOfRedNodes(castNode(node.getRight()));
			}
		}
	...
	private boolean verifyBlackHeight(RBNode<T> node) {
		...
		if (!node.isEmpty()) {
			validLeft = verifyBlackHeight(castNode(node.getLeft()));
			left = blackHeight(castNode(node.getLeft()));
			validRight = verifyBlackHeight(castNode(node.getRight()));
	...
	public void insert(T element) {
		if (element != null) {
			if (!this.isEmpty()) {
				...
				castNode(getRoot()).setColour(Colour.BLACK);
			}
		}
	}
	...
	private void addInArray(RBNode<T> node, RBNode<T>[] array) {
		int count = 0;
